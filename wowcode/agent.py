...
from wowcode.client import LLMClient
from wowcode.context import (
    CompactBoundary,
...
from wowcode.conversation import ConversationManager, ToolResultBlock, ToolUseBlock
from wowcode.conversation import ThinkingBlock as ConvThinkingBlock
from wowcode.memory.auto_memory import MemoryManager
from wowcode.permissions import (
    Decision,
    PermissionChecker,
    PermissionMode,
)
from wowcode.hooks import HookContext, HookEngine, ToolRejectedError
from wowcode.hooks.engine import HookNotification
from wowcode.prompts import build_environment_context, build_plan_mode_reminder, build_system_prompt
from wowcode.tools import ToolRegistry
from wowcode.tools.base import (
    MAX_OUTPUT_CHARS,
    StreamEnd,
    StreamEvent,
    TextDelta,
...
class Agent:
    ...
    def _transcript_path(self) -> str:
        if self.session_id:
            return str(Path(self.work_dir) / ".wowcode" / "sessions" / f"{self.session_id}.jsonl")
        return ""
    ...
    def _get_plan_path(self) -> Path:
        ...
        plans_dir = Path(self.work_dir) / ".wowcode" / "plans"
    ...
    async def run(self, conversation: ConversationManager) -> AsyncIterator[AgentEvent]:
        ...
        while True:
            ...
            # 这样 compact 的阈值判断和 keep_start 计算都基于 budget 缩减后的
            # token 估算，避免因大 tool-result 导致过早或过晚触发压缩。
        ...
        if self.permission_checker:
            ...
            if decision.effect == "ask":
                ...
                if response == PermissionResponse.ALLOW_ALWAYS:
                    from wowcode.permissions.rules import Rule, extract_content
    ...
    def _maybe_persist_or_truncate(self, tool_use_id: str, text: str) -> str:
        from wowcode.context.manager import (
