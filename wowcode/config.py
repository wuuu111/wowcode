...
def load_config(path: Path | None = None) -> AppConfig:
    ...
    home = Path.home()
    candidates = [
        home / ".wowcode" / "config.yaml",
        cwd / ".wowcode" / "config.yaml",
        cwd / ".wowcode" / "config.local.yaml",
    ]
    ...
    if merged is None:
        raise ConfigError(
            "No config file found. Expected .wowcode/config.yaml "
            "in project or ~/.wowcode/config.yaml"
        )
    return merged
